...
Note that although this solution was a top-10 solution, the accuracy of the approach (on "testing.csv") was not revealed by the challenge. So please use it at your own discretion. However, apart from the justifications highlighted in this paper, the effectivenss of model of choice for this problem is based on my personal research experience/experiments (on smart mobility/cities) on such problems (supply/demand prediction), where gradient-boosting methods like LightGBM are still much more effective especially with well-designed features, than current deep learning technique like Graph Convolutional Networks, Graph Attentions, or their variations, etc.
